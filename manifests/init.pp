define windowsfeature($feature_name = $title, $ensure = 'present', $restart = 'false') {
  ...
  if ($ensure == 'present') {
    exec { "add-feature-${title}" :
      ...
      onlyif    => "${windowsfeature::param::powershell::command} -Command \"Import-Module ServerManager; if((Get-WindowsFeature ${features} | where { \$_.InstallState -eq 'Available' }).count -eq 0){ exit 1 }\"",
      logoutput => true,
    }
  } elsif ($ensure == 'absent') {
    exec { "remove-feature-${title}" :
      ...
      path      => "${windowsfeature::param::powershell::path};${::path}",
      unless    => "${windowsfeature::param::powershell::command} -Command \"Import-Module ServerManager; if((Get-WindowsFeature ${features} | where { \$_.InstallState -eq 'Installed' }).count -gt 0){ exit 1 }\"",
      logoutput => true,
    }
  }
}
